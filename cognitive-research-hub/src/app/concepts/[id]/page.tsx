...
import { Concept, ConceptRelationType } from "@/types/concept";

function getRelationLabel(type: ConceptRelationType): string {
  const labels: Record<ConceptRelationType, string> = {
    related: "Related to",
...
export default function ConceptPage() {
  ...
  useEffect(() => {
    async function loadConcept() {
      ...
      const { data: paperConcepts, error } = await supabase
        .from("paper_concepts")
        .select("paper_id, concept_id")
        .eq("concept_id", id);

      if (error) {
        console.error("Failed to load concept-paper relationships:", error);
      }

      // Get all paper IDs connected to this concept
      const paperIds =
        paperConcepts?.map((relation) => relation.paper_id) ?? [];
  ...
  async function handleConnectConcept() {
    ...
    await connectConcepts(concept.id, selectedConceptId, selectedRelationType);

    const updatedConcept = await getConcept(concept.id);
  ...
  async function handleDisconnectConcept(relatedConceptId: string) {
    if (!concept) return;

    await disconnectConcepts(concept.id, relatedConceptId);
  ...
  const relatedConcepts = concept.relations
    .map((relation) => {
      const relatedConcept = allConcepts.find(
        (item) => item.id === relation.conceptId,
      );
  ...
  const availableConcepts = allConcepts.filter(
    (item) =>
      item.id !== concept.id &&
      !concept.relations.some((relation) => relation.conceptId === item.id),
  );
      ...
      <div className="mx-auto max-w-6xl px-6 py-10 lg:px-8 lg:py-12">
          ...
          <div className="space-y-6">
            ...
            <section className="rounded-2xl border border-(--border) bg-(--surface)/60 p-7">
              ...
              <p className="mt-5 max-w-3xl whitespace-pre-wrap text-base leading-8 text-zinc-300">
                {concept.definition || "No definition yet."}
              </p>
            </section>
            {/* My Understanding */}
            ...
            <section className="rounded-2xl border border-(--border) bg-(--surface)/60 p-7">
              ...
              <p className="mt-5 max-w-3xl whitespace-pre-wrap text-base leading-8 text-(--muted)">
                {concept.understanding || "No understanding written yet."}
              </p>
            </section>
            ...
            <section className="rounded-2xl border border-(--border) bg-(--surface)/60 p-7">
              ...
              <p className="mt-5 max-w-3xl whitespace-pre-wrap text-base leading-8 text-(--muted)">
                {concept.openQuestions || "No open questions yet."}
              </p>
            </section>
            ...
            <section className="rounded-2xl border border-(--border) bg-(--surface)/60 p-6">
              ...
              <div className="mt-6 space-y-3">
                {relatedConcepts.length === 0 ? (
                  ...
                  relatedConcepts.map(({ concept: related, type }) => (
                    <div
                      key={related.id}
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <button
                          type="button"
                          onClick={() => handleDisconnectConcept(related.id)}
              ...
              {showRelationPicker && (
                <div className="mt-5 rounded-xl border border-(--border) bg-(--background) p-5">
                  ...
                  <div className="mt-5 space-y-4">
                    <div>
                      <label className="text-xs text-(--muted)">Concept</label>
                      ...
                      <select
                        ...
                        onChange={(event) =>
                          setSelectedRelationType(
                            event.target.value as ConceptRelationType,
                          )
                        }
                        className="mt-2 w-full rounded-lg border border-(--border) bg-(--surface) px-3 py-2.5 text-sm text-zinc-300 outline-none transition"
                      >
                        ...
                        <option value="contrasts">Contrasts with</option>

                        <option value="part-of">Part of</option>

                        <option value="causes">Causes / influences</option>
        ...
        <section className="mt-6 rounded-2xl border border-(--border) bg-(--surface)/60 p-7">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 text-lg font-semibold">Connected Papers</h2>
        ...
        <section className="mt-6 border-t border-zinc-900 pt-6">
          <div className="flex flex-wrap gap-x-8 gap-y-3 text-xs text-zinc-700">
            ...
            <span>
              {concept.paperIds.length}{" "}
              {concept.paperIds.length === 1 ? "paper" : "papers"}
            </span>

            <span>
              {relatedConcepts.length}{" "}
              {relatedConcepts.length === 1 ? "relationship" : "relationships"}
            </span>
          </div>
        </section>
      </div>
