...
export default function PaperPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (error) {
        console.error("Failed to load paper concepts:", error);
      }
  ...
  async function handleAddConcept(concept: Concept) {
    ...
    await connectPaperToConcept(paper.id, concept.id);
    ...
    if (error) {
      console.error("Failed to reload paper concepts:", error);
      return;
    }

    setPaper({
      ...paper,
      conceptIds: connections?.map((connection) => connection.concept_id) ?? [],
    });
  ...
  async function handleRemoveConcept(concept: Concept) {
    ...
    await disconnectPaperFromConcept(paper.id, concept.id);
  ...
  const availableConcepts = concepts.filter(
    (concept) => !(paper.conceptIds ?? []).includes(concept.id),
  );
  ...
  return (
    ...
    "
        ...
        <div className="mt-12 space-y-6">
          ...
          <ResearchSection title="Method" content={paper.method} />

          <ResearchSection title="Key Results" content={paper.results} />
          ...
          <ResearchSection title="My Critique" content={paper.critique} />
  ...
  return (
    <section className="rounded-2xl border border-(--border) bg-(--surface) p-6">
      ...
      {content ? (
        ...
        <p className="mt-4 italic text-(--subtle)">No notes yet.</p>
      )}
    </section>
  );
}
