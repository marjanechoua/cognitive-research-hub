...
export default function ResearchAnalytics({
  ...
  const relationData = Object.entries(relationCounts).sort(
    (a, b) => b[1] - a[1],
  );
  ...
  const connectedConcepts = [...concepts]
    .map((concept) => ({
      ...concept,
      connectionCount: concept.relations.length + concept.paperIds.length,
    }))
    .sort((a, b) => b.connectionCount - a.connectionCount)
    .slice(0, 5);

  const activity = getActivity(concepts, papers);
    ...
    <section className="mt-6 space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 max-w-2xl text-sm leading-6 text-(--muted)">
          See how your concepts are connected and how your research library is
          growing over time.
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        >
          ...
          {relationData.length === 0 ? (
            <div className="mt-8 rounded-xl border border-dashed border-(--border) px-5 py-8 text-center">
            ...
            <div className="mt-7 space-y-5">
              {relationData.map(([type, count]) => {
                const percentage = (count / maxRelationCount) * 100;
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span
                          ...
                          style={{
                            backgroundColor:
                              relationColors[type] ?? "var(--accent)",
                          }}
                        />
          ...
          <div className="flex h-40 items-end gap-2 sm:gap-4">
            {activity.map((day) => {
              const height =
                day.count === 0
                  ? 4
                  : Math.max(8, (day.count / maxActivity) * 100);
...
function getActivity(concepts: Concept[], papers: Paper[]) {
  const days = [];
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const conceptCount = concepts.filter((concept) => {
      const created = new Date(concept.createdAt);
